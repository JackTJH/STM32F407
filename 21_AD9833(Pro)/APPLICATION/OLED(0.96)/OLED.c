...
/**
  ...
  * 返 回 值：无
  */
void OLED_WriteCommand(uint8_t Command,I2C_TYPE i2c_type,I2CX_Typedef i2cx)
{
	if(i2c_type == S_I2C)
	{
		JI2C_Start(i2cx);
		JI2C_SendByte(0x78,i2cx);
		JI2C_WaitAck(i2cx);
		
		JI2C_SendByte(0x00,i2cx);
		JI2C_WaitAck(i2cx);
		
		JI2C_SendByte(Command,i2cx);
		JI2C_WaitAck(i2cx);
...
/**
  ...
  * 返 回 值：无
  */
void OLED_WriteData(uint8_t *Data, uint8_t Count,I2C_TYPE i2c_type,I2CX_Typedef i2cx)
{
	uint8_t i;
	...
	{
		JI2C_Start(i2cx);
		JI2C_SendByte(0x78,i2cx);		//发送OLED的I2C从机地址
		JI2C_WaitAck(i2cx);

		JI2C_SendByte(0x40,i2cx);		//控制字节，给0x40，表示即将写数据
		JI2C_WaitAck(i2cx);

		/*循环Count次，进行连续的数据写入*/
		for (i = 0; i < Count; i ++)
		{
			JI2C_SendByte(Data[i],i2cx);	//依次发送Data的每一个数据
			JI2C_WaitAck(i2cx);
		}
		JI2C_Stop(i2cx);	
	}
	else
	{
		...
		I2C_SendData(JI2X[i2cx], 0x40);
		JI2CXX_WaitEvent(i2cx,I2C_EVENT_MASTER_BYTE_TRANSMITTING);
		
		for(i = 0; i < Count; i++)
		{
			I2C_SendData(JI2X[i2cx], Data[i]);
			JI2CXX_WaitEvent(i2cx,I2C_EVENT_MASTER_BYTE_TRANSMITTING);
		}	
		I2C_GenerateSTOP(JI2X[i2cx], ENABLE);
	}
...
/**
  ...
  * 说    明：使用前，需要调用此初始化函数
  */
void OLED_Init(I2C_TYPE i2c_type,I2CX_Typedef i2cx)
{
	/*写入一系列的命令，对OLED进行初始化配置*/
	OLED_WriteCommand(0xAE,i2c_type,i2cx);	//设置显示开启/关闭，0xAE关闭，0xAF开启
	
	OLED_WriteCommand(0xD5,i2c_type,i2cx);	//设置显示时钟分频比/振荡器频率
	OLED_WriteCommand(0x80,i2c_type,i2cx);	//0x00~0xFF
	
	OLED_WriteCommand(0xA8,i2c_type,i2cx);	//设置多路复用率
	OLED_WriteCommand(0x3F,i2c_type,i2cx);	//0x0E~0x3F
	
	OLED_WriteCommand(0xD3,i2c_type,i2cx);	//设置显示偏移
	OLED_WriteCommand(0x00,i2c_type,i2cx);	//0x00~0x7F
	
	OLED_WriteCommand(0x40,i2c_type,i2cx);	//设置显示开始行，0x40~0x7F
	
	OLED_WriteCommand(0xA1,i2c_type,i2cx);	//设置左右方向，0xA1正常，0xA0左右反置
	
	OLED_WriteCommand(0xC8,i2c_type,i2cx);	//设置上下方向，0xC8正常，0xC0上下反置

	OLED_WriteCommand(0xDA,i2c_type,i2cx);	//设置COM引脚硬件配置
	OLED_WriteCommand(0x12,i2c_type,i2cx);
	
	OLED_WriteCommand(0x81,i2c_type,i2cx);	//设置对比度
	OLED_WriteCommand(0xFF,i2c_type,i2cx);	//0x00~0xFF

	OLED_WriteCommand(0xD9,i2c_type,i2cx);	//设置预充电周期
	OLED_WriteCommand(0xF1,i2c_type,i2cx);

	OLED_WriteCommand(0xDB,i2c_type,i2cx);	//设置VCOMH取消选择级别
	OLED_WriteCommand(0x30,i2c_type,i2cx);

	OLED_WriteCommand(0xA4,i2c_type,i2cx);	//设置整个显示打开/关闭

	OLED_WriteCommand(0xA6,i2c_type,i2cx);	//设置正常/反色显示，0xA6正常，0xA7反色

	OLED_WriteCommand(0x8D,i2c_type,i2cx);	//设置充电泵
	OLED_WriteCommand(0x14,i2c_type,i2cx);

	OLED_WriteCommand(0xAF,i2c_type,i2cx);	//开启显示
	
	OLED_Clear();				//清空显存数组
	OLED_Update(i2c_type,i2cx);				//更新显示，清屏，防止初始化后未显示内容时花屏
}
...
/**
  ...
  */
void OLED_SetCursor(uint8_t Page, uint8_t X,I2C_TYPE i2c_type,I2CX_Typedef i2cx)
{
...
//	X += 2;
	...
	/*通过指令设置页地址和列地址*/
	OLED_WriteCommand(0xB0 | Page,i2c_type,i2cx);					//设置页位置
	OLED_WriteCommand(0x10 | ((X & 0xF0) >> 4),i2c_type,i2cx);	//设置X位置高4位
	OLED_WriteCommand(0x00 | (X & 0x0F),i2c_type,i2cx);			//设置X位置低4位
}
...
/**
  ...
  */
void OLED_Update(I2C_TYPE i2c_type,I2CX_Typedef i2cx)
{
	uint8_t j;
	/*遍历每一页*/
	for (j = 0; j < 8; j ++)
	{
		/*设置光标位置为每一页的第一列*/
		OLED_SetCursor(j, 0,i2c_type,i2cx);
		/*连续写入128个数据，将显存数组的数据写入到OLED硬件*/
		OLED_WriteData(OLED_DisplayBuf[j], 128,i2c_type,i2cx);
	}
}
...
/**
  ...
  *           故调用显示函数后，要想真正地呈现在屏幕上，还需调用更新函数
  */
void OLED_UpdateArea(int16_t X, int16_t Y, uint8_t Width, uint8_t Height,I2C_TYPE i2c_type,I2CX_Typedef i2cx)
{
	int16_t j;
	int16_t Page, Page1;
	...
	for (j = Page; j < Page1; j ++)
	{
		...
		{
			/*设置光标位置为相关页的指定列*/
			OLED_SetCursor(j, X,i2c_type,i2cx);
			/*连续写入Width个数据，将显存数组的数据写入到OLED硬件*/
			OLED_WriteData(&OLED_DisplayBuf[j][X], Width,i2c_type,i2cx);
		}
	}
}
