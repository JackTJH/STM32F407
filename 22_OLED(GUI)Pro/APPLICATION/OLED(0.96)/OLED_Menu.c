	...
	switch(oper)
	{
		...
		case BUTTON_UNCONDITION_INSERT:
			if(menu.menu_flag < MENU_LEVEL)
			{
				...
				{
					Menu_Input(0,menu,menu.sub3_menu,0,2);
				}
			}
			break;
		case BUTTON_INSERT:
			menu.item[menu.cur_item].func();
			break;
		case BUTTON_EXTI:
			Menu_Return();
			break;
...
void Menu_Input(uint8_t cur_item_ok,MENU_t menu_ok,MENU_t *sub_menu_ok,\
								uint8_t cur_item_next_ok,uint8_t menu_flag_ok)
{	
...
void Menu_Image_Display(uint8_t image_index)
{
	...
	const uint8_t *image_data_level_1[] = {ear,game,chess,fish,music,badminton};			//一级菜单图片
