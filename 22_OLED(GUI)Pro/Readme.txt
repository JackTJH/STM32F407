...
15.当存储字符串时候一定要加const不然存不进去
const uint8_t Tx_Buffer[] = "JackTJH@nuaa.edu.cn";这样就可以
uint8_t Tx_Buffer[] = "JackTJH@nuaa.edu.cn";这样不行
16.在用软件模拟的I2C中读取E2PROM写入需要加一个2ms的延时不然写不进去
17.这里硬件读取I2C有bug，AT24CXX_ReadPage读取是没问题的，在这里AT24CXX_ReadBuffer读取卡死（暂未解决）
已经解决提前nack和stop
18	JI2C_Start(i2cx);
...
在按页读取的时候while(JI2C_WaitAck(i2cx) != SET);不注释掉字符串写不进去，如果注释掉小数读写不对
19在读去多个字节的时候，AT24C02无论是读还是写都必须先把STOP和ACK放在读取最后一个字节前并且采取while结构
如下：
JI2C_Start(i2cx);
JI2C_SendByte(0xA0,i2cx);
JI2C_WaitAck(i2cx);
...
在OLED中写多个字节的时候需用for循环结构并且不需要事先stop

硬件I2C暂时只支持100kHZ

20硬件SPI在MFRC522中写入有问题(问题已经解决)
初始化分频太小SPI_InitStructure.SPI_BaudRatePrescaler = SPI_BaudRatePrescaler_64;
