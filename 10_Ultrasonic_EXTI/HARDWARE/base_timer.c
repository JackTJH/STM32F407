...
void BASE_Timer7_Init(void)
{
	...
	RCC_APB1PeriphClockCmd(BASE_TIMER7_RCC,ENABLE);//打开基本定时器时钟
	TIM_TimeBaseInitStructure.TIM_CounterMode = TIM_CounterMode_Up;//根据手册这里基本定时器TIM6和TIM7只能向上计数
	TIM_TimeBaseInitStructure.TIM_Period = 10000-1;
	TIM_TimeBaseInitStructure.TIM_Prescaler = 8200-1;
	TIM_TimeBaseInit(BASE_TIMER7,&TIM_TimeBaseInitStructure);
	TIM_ITConfig(BASE_TIMER7,TIM_IT_Update,ENABLE);//打开中断源
	
	//配置NVIC
	NVIC_InitTypeDef NVIC_InitStructure;
	NVIC_InitStructure.NVIC_IRQChannel = NVIC_IRQ_BASE_TIMER7;
	NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
	NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = NVIC_BASE_TIMER7_PREEMPTION_PRIORITY;
	NVIC_InitStructure.NVIC_IRQChannelSubPriority = NVIC_BASE_TIMER7_SUB_PRIORITY;
	NVIC_Init(&NVIC_InitStructure);
//	TIM_Cmd(BASE_TIMER7,ENABLE);//打开定时器
}	
